...
def Color_least_frequent(df, column_name, new_column_name, top_n=8,wb = False):
    ...
    color_list = ['זהב','ירוק','אדום','כסף','לבן','אפור','כחול','שחור']
    if wb:
        df[new_column_name] = df[new_column_name].apply(merge_similar_categories)
        # Iterate over each value in the column and apply logic
        df[new_column_name] = df[new_column_name].apply(lambda x: x if x in color_list else 'else')
